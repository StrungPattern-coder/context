/**
 * ═══════════════════════════════════════════════════════════════════════════════
 *         RAL MASTER INJECTOR - Claude v4.6 (Single Source of Truth)
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * The definitive, self-contained Claude integration. Zero external dependencies.
 * 
 * EXTREME INTELLIGENCE FEATURES:
 *   1. Flicker-Free Ghost Scrubbing  - Hidden span + badge, no XML flash
 *   2. Brute-Force ProseMirror Sync  - Synthetic Tickle for 100% reliability
 *   3. Smart Adaptive Badges         - Edit/Frustrated/Normal variants (Cyberpunk)
 *   4. Cinematic Animations          - Glitch materialize effect
 *   5. Behavioral Orb                - Max z-index, pointer-events safe
 * 
 * @version 4.6.0
 * @license MIT
 */
...
(function() {
  ...
  console.log(`RAL Master Injector v${RAL_VERSION} [Claude] loaded`);
  ...
  function setupGhostContextScrubber() {
    ...
    console.log('RAL v4.6 [Claude]: Flicker-Free Ghost Scrubber active');
  }
  ...
  // ════════════════════════════════════════════════════════════════════════════
  //            FEATURE 2: PROSEMIRROR STATE SYNC
  // ════════════════════════════════════════════════════════════════════════════
    ...
    try {
      ...
      if (element.isContentEditable) {
        ...
        if (getInputValue(element) !== newValue) {
          ...
          element.dispatchEvent(new InputEvent('input', { bubbles: true, cancelable: true, inputType: 'insertText', data: newValue }));
        }
      } else {
      ...
      console.error('RAL [Claude]: Sync error:', e);
      return false;
    }
  }
  ...
  function validateSendButtonState(inputElement) {
    ...
    const input = inputElement || findInput();
    if (!btn || !input) return;
  ...
  async function injectContext(element) {
    ...
    if (State.editMode) contextPrefix = contextPrefix.replace('<reality_context', '<reality_context type="edit_context"');
    ...
    if (success) {
      ...
      console.log('RAL v4.6 [Claude]: Context injected!');
    }
    return success;
  }

  // ════════════════════════════════════════════════════════════════════════════
  //                          DOM HELPERS (CLAUDE-SPECIFIC)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .ral-context-badge {
        ...
        box-shadow: 0 0 8px var(--badge-accent-dim), inset 0 0 20px rgba(0, 0, 0, 0.5);
      ...
      .ral-badge-text { text-shadow: 0 0 8px var(--badge-accent-dim); position: relative; z-index: 1; }
      ...
      @keyframes ral-cyber-materialize {
        0% { opacity: 0; transform: translateX(-20px) skewX(-5deg); filter: blur(4px); }
        30% { opacity: 1; transform: translateX(3px) skewX(2deg); filter: blur(0); }
      ...
      .ral-orb:hover .ral-orb-status { opacity: 1; transform: scale(1); }
  ...
  async function init() {
    ...
    console.log(`RAL Master Injector v${RAL_VERSION} [Claude]: Ready! 🧠⚡`);
  }
