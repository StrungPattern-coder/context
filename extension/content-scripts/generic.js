/**
 * ═══════════════════════════════════════════════════════════════════════════════
 *         RAL MASTER INJECTOR - Generic v4.6 (Single Source of Truth)
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * Universal AI chatbot integration with heuristic input detection.
 * Works with localhost deployments, custom AI UIs, and unknown platforms.
...
(function() {
  ...
  console.log(`RAL Master Injector v${RAL_VERSION} [Generic] loaded`);
  ...
  function setupGhostContextScrubber() {
    ...
    const container = document.querySelector('[class*="message"]') || document.querySelector('[class*="chat"]') || document.querySelector('main') || document.body;
    scrubber.observe(container, { childList: true, subtree: true });

    // Initial scrub
    scrubContextFromElement(document.body);
    console.log('RAL v4.6 [Generic]: Flicker-Free Ghost Scrubber active');
  }
  ...
  async function injectContext(element) {
    ...
    if (success) {
      ...
      console.log('RAL v4.6 [Generic]: Context injected!');
    }
    return success;
  }

  // ════════════════════════════════════════════════════════════════════════════
  //                      DOM HELPERS (HEURISTIC DETECTION)
  // ════════════════════════════════════════════════════════════════════════════

  // Chat-related keywords for heuristic detection
  const CHAT_KEYWORDS = [
    'chat', 'message', 'prompt', 'ask', 'send', 'input', 'query', 'question',
    'conversation', 'assistant', 'ai', 'bot', 'llm', 'gpt', 'claude', 'model'
  ];

  function hasKeywordMatch(element) {
    const attrs = [
      element.placeholder,
    ...
    return CHAT_KEYWORDS.some(kw => attrs.includes(kw));
  }

  function findInput() {
    // Priority 1: Common AI chat selectors
    const prioritySelectors = [
      // Gradio (common for HuggingFace demos)
    ...
    // Priority 2: Heuristic search for textareas
    const textareas = document.querySelectorAll('textarea');
    for (const textarea of textareas) {
      if (textarea.offsetParent !== null && hasKeywordMatch(textarea)) {
        return textarea;
      }
    }

    // Priority 3: Contenteditable divs with chat-like attributes
    const editables = document.querySelectorAll('div[contenteditable="true"]');
    for (const el of editables) {
      if (el.offsetParent !== null && hasKeywordMatch(el)) {
        return el;
      }
    }

    // Priority 4: Any visible textarea (fallback)
    for (const textarea of textareas) {
  ...
  function findSendButton() {
    // Priority 1: Common send button selectors
    const prioritySelectors = [
      'button[type="submit"]',
    ...
    // Priority 2: Button near input with SVG (common pattern)
    const input = findInput();
    if (input) {
      const form = input.closest('form');
      const container = form || input.parentElement?.parentElement?.parentElement;
      
      if (container) {
        const buttons = container.querySelectorAll('button');
        for (const btn of buttons) {
          if (btn.offsetParent !== null) {
            // Check for send icon (paper plane, arrow, etc.)
            const hasSendIcon = btn.querySelector('svg') !== null;
            const hasSubmitType = btn.type === 'submit';
            const hasSendText = btn.textContent.toLowerCase().match(/send|submit|go|ask/);
            
            if (hasSendIcon || hasSubmitType || hasSendText) {
              return btn;
            }
          }
        }
    ...
    try {
      ...
      const triggerSend = () => {
        ...
        if (btn && !btn.disabled) {
          ...
          // Fallback: submit form or dispatch Enter
          const form = input.closest('form');
          if (form) {
  ...
  async function init() {
    ...
    console.log(`RAL Master Injector v${RAL_VERSION} [Generic]: Ready! 🧠⚡`);
  }
